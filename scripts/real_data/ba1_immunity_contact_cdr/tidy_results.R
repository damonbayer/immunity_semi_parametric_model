target_fit_id <- ifelse(length(commandArgs(trailingOnly=T)) == 0, 1, as.integer(commandArgs(trailingOnly=T[1])))
...
target_county_id <- fit_info[["county_id"]]
...
tidy_prior_generated_quantities <- 
  ...
  mutate(time = if_else(str_detect(name, "seq"),
                        variant_2_import_time + (time - 1) / 7,
                        time)) %>% 
...
phi_draws <-
  ...
  mutate(phi = if_else(phi > 1e3, 1e3, phi))
...
posterior_predictive_score_setup <-
  ...
  select(sample = .draw, prediction = value, name, date, weeks_ahead) %>%
  left_join(dat_tidy) %>%
  mutate(model = fit_info[["fit_id"]]) %>%
  select(date, model, target_type = name, weeks_ahead, prediction, sample, true_value)
...
posterior_predictive_score_nbinom <-
  ...
  unnest_wider(nbinom) %>%
  group_by(date, model, target_type, weeks_ahead) %>%
