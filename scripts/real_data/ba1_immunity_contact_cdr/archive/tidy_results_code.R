...
posterior_predictive_score_setup <-
  ...
  select(date, model, target_type = name, max_t, weeks_ahead, prediction, sample, true_value)
...
posterior_predictive_score_nbinom <-
  ...
  unnest_wider(nbinom) %>%
  group_by(date, model, target_type, max_t, weeks_ahead) %>%
