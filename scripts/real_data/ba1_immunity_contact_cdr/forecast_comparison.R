...
model_table_subset <- read_csv(path(simulation_dir, "model_table.csv")) %>% 
  filter(as.integer(factor(county_id)) == target_data_id) %>% 
  mutate(across(c(immunity_model, `R₀_model`, CDR_model),
                ~case_when(.x == "seq-informed" ~ "seq",
                           .x == "constant" ~ "const",
                           TRUE ~ .x))) %>% 
  mutate(model_description = glue("R₀: {`R₀_model`}\nimm: {immunity_model}\nCDR: {CDR_model}") %>% as.character(),
         model_description_one_line = glue("R0_{`R₀_model`}_imm_{immunity_model}_CDR_{CDR_model}") %>% as.character())

target_county <- model_table_subset[["county"]][[1]] %>% str_c(., if_else(. == "California", "", " County"))
target_county_id <- model_table_subset[["county_id"]][[1]]
...
plot_forecast_comparison <- function(target_type) {
  ...
  ggplot(mapping = aes(time, value)) +
    ...
    ggtitle(glue("Forecast Comparison for {str_to_title(str_replace_all(target_type, '_', ' '))}"),
            subtitle = glue("{str_to_title(target_county)} Data"))
}
...
plot_scalar_generated_quantities_by_forecast_time <- function(target_model_description_one_line) {
  ...
  tidy_generated_quantities_tbl %>%
    ...
    filter(model_description == target_model_description) %>%
    ggplot(aes(time_to_date(max_time), value, ymin = .lower, ymax = .upper, color = distribution)) +
...
plot_vector_generated_quantities_by_forecast_time <- function(target_model_description_one_line) {
  ...
  tidy_generated_quantities_tbl %>%
    ...
    ggplot(aes(time_to_date(time), value, color = max_t, group = max_t)) +
    facet_wrap(~name, scales = "free_y") +
...
plot_single_generated_quantities <- function(target_model_description_one_line, target_max_t) {
  ...
  tidy_generated_quantities_tbl %>%
    ...
    filter(!is.na(time),
           ...
           max_t == target_max_t,
           .width == 0.8) %>%
    ggplot(mapping = aes(time_to_date(time), value, ymin = .lower, ymax = .upper, color = distribution, fill = distribution)) +
    facet_wrap(.~name, scales = "free_y") +
    geom_lineribbon(alpha = 0.25) +
    geom_vline(xintercept = target_max_t, linetype = "dashed") +
    scale_x_date("Date") +
    scale_y_continuous(labels = comma) +
    ggtitle(glue("{str_replace_all(target_model_description, '\n', ', ')} Generated Quantities at t = {time_to_date(target_max_t)}"),
            subtitle = glue("{str_to_title(target_county)} Data"))
}
...
plot_compare_posterior_generated_quantities <- function(target_max_t) {
  tidy_generated_quantities_tbl %>%
    ...
    filter(!is.na(time),
           ...
           max_t == target_max_t,
           .width == 0.8) %>%
    ggplot(mapping = aes(time_to_date(time), value, ymin = .lower, ymax = .upper, color = model_description, fill = model_description, group = .width)) +
    facet_wrap(.~name, scales = "free_y") +
    geom_lineribbon(alpha = 0.25) +
    geom_vline(xintercept = time_to_date(target_max_t), linetype = "dashed") +
    # my_theme +
    scale_x_date("Date") +
    scale_y_continuous(labels = comma) +
    ggtitle(glue("Posterior Generated Quantities at t = {time_to_date(target_max_t)}"),
            subtitle = glue("{str_to_title(target_county)} Data"))
}
...
forecast_comparison_plots <-
  ...
  mutate(file_path = path("figures", experiment_name, glue("{target_county %>% str_to_lower() %>% str_replace_all(' ', '_')}"), glue("forecast_comparison_{target_type}_plot"), ext = "pdf"),
         figure = future_map(target_type, plot_forecast_comparison)) %>%
  augment_figure_tbl()

forecast_metrics_comparison_plots <-
  tibble(target_type = all_target_types) %>%
  mutate(file_path = path("figures", experiment_name, glue("{target_county %>% str_to_lower() %>% str_replace_all(' ', '_')}"), glue("forecast_metrics_comparison_{target_type}_plot"), ext = "pdf"),
...
scalar_generated_quantities_by_forecast_time_plots <-
  ...
  mutate(file_path = path("figures", experiment_name, glue("{target_county %>% str_to_lower() %>% str_replace_all(' ', '_')}"), glue("scalar_generated_quantities_by_forecast_time_{target_model_description_one_line}_plot"), ext = "pdf"),
...
vector_generated_quantities_by_forecast_time_plots <-
  tibble(target_model_description_one_line = all_model_description_one_lines) %>%
  mutate(file_path = path("figures", experiment_name, glue("{target_county %>% str_to_lower() %>% str_replace_all(' ', '_')}"), glue("vector_generated_quantities_by_forecast_time_{target_model_description_one_line}_plot"), ext = "pdf"),
...
single_generated_quantities_plots <-
  ...
  mutate(file_path = path("figures", experiment_name, glue("{target_county %>% str_to_lower() %>% str_replace_all(' ', '_')}"), glue("single_generated_quantities_{target_model_description_one_line}_{target_max_t}_plot"), ext = "pdf"),
...
compare_posterior_generated_quantities_plots <-
  ...
  distinct(max_t) %>%
  rename_with(~str_c("target_", .x)) %>%
  mutate(file_path = path("figures", experiment_name, glue("{target_county %>% str_to_lower() %>% str_replace_all(' ', '_')}"), glue("compare_posterior_generated_quantities_{target_max_t}_plot"), ext = "pdf"),
...
single_posterior_predictive_plots <-
  ...
  distinct(model_description_one_line, max_t) %>%
  rename_with(~str_c("target_", .x)) %>%
  mutate(file_path = path("figures", experiment_name, glue("{target_county %>% str_to_lower() %>% str_replace_all(' ', '_')}"), glue("single_posterior_predictive_{target_model_description_one_line}_{target_max_t}_plot"), ext = "pdf"),
...
peak_assessment_plots <-
  ...
  mutate(file_path = path("figures", experiment_name, glue("{target_county %>% str_to_lower() %>% str_replace_all(' ', '_')}"), glue("peak_assessment_{target_peak_type}_plot"), ext = "pdf"),
         figure = future_map(target_peak_type, plot_peak_assessment)) %>%
  augment_figure_tbl()

peak_metrics_plots <-
  tibble(target_peak_type = all_peak_types) %>%
  mutate(file_path = path("figures", experiment_name, glue("{target_county %>% str_to_lower() %>% str_replace_all(' ', '_')}"), glue("peak_metrics_{target_peak_type}_plot"), ext = "pdf"),
...
# Save Figures ------------------------------------------------------------
dir_create(path("figures", experiment_name, glue("{target_county %>% str_to_lower() %>% str_replace_all(' ', '_')}")))
dir_create(path("figures", experiment_name, glue("{target_county %>% str_to_lower() %>% str_replace_all(' ', '_')}"), all_model_description_one_lines))


# Save the compute time plot
save_plot(filename = path("figures", experiment_name, glue("{target_county %>% str_to_lower() %>% str_replace_all(' ', '_')}"), "compute_time", ext = "pdf"),
...
non_single_plot_tbl_names %>%
  walk(~{
    ...
    collected_plot_path <- path("figures", experiment_name, glue("{target_county %>% str_to_lower() %>% str_replace_all(' ', '_')}"), str_c("all_", .x), ext = "pdf")
...
merge_single_plots_fn <- function(single_plot_tbl_name) {
  ...
  single_plot_tbl %>%
    ...
    group_walk(~{
      ...
      collected_plot_path <- path("figures", experiment_name, glue("{target_county %>% str_to_lower() %>% str_replace_all(' ', '_')}"), .y, str_c("all_", single_plot_tbl_name, "_", .y), ext = "pdf")
