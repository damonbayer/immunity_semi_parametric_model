...
experiment_name <-  "compare_inference_unknown_variant_proportion_ihr_contact"
...
plot_scalar_generated_quantities_by_forecast_time <- function(target_model_name) {
  ...
  tidy_generated_quantities_tbl %>% 
    ...
    geom_hline(data = true_generated_quantities %>% 
                 mutate(name = name %>% 
                          str_remove_all("[\u2080-\u2089]")) %>% 
                 filter(name %in% available_generated_quantities_names),
