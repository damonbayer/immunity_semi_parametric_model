...
model_sample = seirs_multivariant(data_zero_cases, data_zero_hospitalizations, data_zero_icu, data_zero_deaths, obstimes, prob, Tsit5(), 1e-11, 1e-8, init_index_to_report)
model_generated_quantities = seirs_multivariant(data_zero_cases, data_zero_hospitalizations, data_zero_icu, data_zero_deaths, obstimes, prob, Tsit5(), 1e-11, 1e-8, init_index_to_report)
model_predict = seirs_multivariant(data_missing_cases, data_missing_hospitalizations, data_missing_icu, data_missing_deaths, obstimes, prob, Tsit5(), 1e-11, 1e-8, init_index_to_report)

n_datasets = 200

Random.seed!(1)
prior_samples = sample(model_sample, Prior(), n_datasets)
