...
data_new_cases_full, data_hospitalizations_full, data_icu_full, data_new_deaths_full = map(x -> Int.(vec(vcat(get(simulated_data, x)[x]...))), [:data_new_cases, :data_hospitalizations, :data_icu, :data_new_deaths])
