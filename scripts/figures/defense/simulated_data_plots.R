...
proportion_novel_variant_simulated_data_plot <- 
  ...
  mutate(takeover_speed = fct_rev(takeover_speed)) %>% 
...
save_plot_target_asp(filename = path(defense_figure_dir, "proportion_novel_variant_simulated_data_plot", ext = "pdf"),
          plot = proportion_novel_variant_simulated_data_plot,
          base_height = 4.7,
          base_asp = slide_target_asp / 2)
...
combined_hosp_data_plot <- 
  ggplot(combined_hosp_data, aes(time, value)) +
  facet_wrap(~takeover_speed, scales = "free_x", labeller = as_labeller(~glue("{str_to_title(.x)} Takeover Speed Data"))) +
  geom_line() +
  geom_point() +
  geom_rect(data = distinct(combined_hosp_data, takeover_speed, first_forecast_target_t, last_forecast_target_t), mapping = aes(xmin = first_forecast_target_t, xmax = last_forecast_target_t, ymin = -Inf, ymax = Inf), alpha = 0.5, inherit.aes = F) +
  scale_x_continuous(name = "Time") +
  scale_y_continuous(name = "Hospital Occupancy", labels = comma) +
  ggtitle("Simulated Data")

save_plot_target_asp(filename = path(defense_figure_dir, "combined_hosp_data_plot", ext = "pdf"), plot = combined_hosp_data_plot, ncol = 3)
