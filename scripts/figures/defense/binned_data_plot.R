...
figure_tbl <- 
  ...
  mutate(figure = pmap(list(county,
                    ...
                    ~{
                      all_dat %>% 
                        ...
                        scale_x_date(name = "Date", date_labels = "%b '%y", date_breaks = "2 months",
  ...
  mutate(file_path = path(defense_figure_dir, glue("{county_name %>% str_to_lower() %>% str_replace_all(' ', '_')}_binned_data_plot"), ext = "pdf")) %>% 
  select(figure, n_row, n_col, file_path)


abbreviated_figure_tbl <- 
  model_table %>% 
  group_by(county) %>% 
  summarize(min_max_t = min(max_t),
            max_max_t = max(max_t)) %>% 
  ...
  mutate(figure = pmap(list(county,
                       ...
                       ~{
                         all_dat %>% 
                           ...
                           geom_point() +
                           scale_y_continuous(name = "Value", labels = comma) +
                           scale_x_date(name = "Date", date_labels = "%b '%y", breaks = ymd(c("2021-06-01", "2021-08-01", "2021-10-01", "2021-12-01", "2022-02-01"))
                                        # guide = guide_axis(angle = 90)
                           ) +
                           ggtitle(glue("{..4} Omicron BA.1 Wave"))
                       })) %>% 
  ...
  unnest_wider(figure_dims) %>%
  rename(n_row = ROW, n_col = COL) %>% 
  mutate(file_path = path(defense_figure_dir, glue("{county_name %>% str_to_lower() %>% str_replace_all(' ', '_')}_abbreviated_binned_data_plot"), ext = "pdf")) %>% 
  select(figure, n_row, n_col, file_path)

pwalk(as.list(figure_tbl), ~save_plot_target_asp(filename = ..4, plot = ..1, ncol = ..3, nrow = ..2, base_height = 1.75, base_asp = 1730 / 420))

pwalk(as.list(abbreviated_figure_tbl), ~save_plot_target_asp(filename = ..4, plot = ..1, ncol = ..3, nrow = ..2, base_height = 2, base_asp = slide_target_asp / 2))
