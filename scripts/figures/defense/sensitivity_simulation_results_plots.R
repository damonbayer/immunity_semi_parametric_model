# compute time plot across data sets
library(tidyverse)
library(glue)
library(fs)
library(furrr)
...
model_table <- read_csv(path(simulation_dir, "model_table.csv")) %>%
  ...
  filter(!is.na(prior_takeover_speed)) %>%
  mutate(
    model_description_one_line = glue("prior_takeover_speed_{`prior_takeover_speed`}") %>% as.character(),
    # model_description = glue("$R_0(t)$: {`R₀_model`}, $1/\\kappa(t)$: {immunity_model}") %>% TeX(output = "character")
    model_description = glue("Prior:~{str_to_title(`prior_takeover_speed`)}")
  ) %>% 
  mutate(
  ...
  mutate(prior_takeover_speed = fct_inorder(prior_takeover_speed))
...
plot_forecast_comparison <- function(target_type, target_data_takeover_speed) {
  tmp_tidy_posterior_predictive <-
    ...
    filter(data_takeover_speed == target_data_takeover_speed)
  ...
  ggplot(mapping = aes(time, value)) +
    ...
    ggtitle(glue("Forecast Comparison for {my_sim_labeller[target_type]}"),
            subtitle = glue("{str_to_title(target_data_takeover_speed)} Takeover Speed Data")
    )
}
...
plot_crps_comparison <- function(target_target_type) {
  tidy_posterior_predictive_score_tbl %>%
    ...
    facet_grid2(forecast_horizon~data_takeover_speed,
                independent = "y",
                scales = "free",
                labeller = labeller(data_takeover_speed = ~glue("{str_to_title(.x)} Takeover Speed Data"))
    ) +
    # stat_smooth(
    #   geom = "line",
    #   formula = y ~ 1,
...
forecast_comparison_plots <-
  ...
  mutate(
    file_path = path(defense_figure_dir, glue("sensitivity_simulated_forecast_comparison_{target_type}_{target_data_takeover_speed}_plot"), ext = "pdf"),
...
crps_comparison_plots <-
  ...
  mutate(
    file_path = path(defense_figure_dir, glue("sensitivity_simulated_crps_comparison_{target_type}_plot"), ext = "pdf"),
...
crps_comparison_boxplot_plots <-
  tibble(target_type = all_target_types) %>%
  mutate(
    file_path = path(defense_figure_dir, glue("sensitivity_simulated_crps_comparison_boxplot_{target_type}_plot"), ext = "pdf"),
...
crps_comparison_dotplot_plots <-
  tibble(target_type = all_target_types) %>%
  mutate(
    file_path = path(defense_figure_dir, glue("sensitivity_simulated_crps_comparison_dotplot_{target_type}_plot"), ext = "pdf"),
...
peak_assessment_plots <-
  ...
  mutate(
    file_path = path(defense_figure_dir, glue("sensitivity_simulated_peak_assessment_{peak_type}_plot"), ext = "pdf"),
...
peak_crps_plots <-
  tibble(x = 1) %>%
  mutate(
    file_path = path(defense_figure_dir, glue("sensitivity_simulated_peak_crps_plot"), ext = "pdf"),
...
peak_crps_boxplot_plots <-
  tibble(x = 1) %>%
  mutate(
    file_path = path(defense_figure_dir, glue("sensitivity_simulated_peak_crps_boxplot_plot"), ext = "pdf"),
...
peak_crps_dotplot_plots <-
  tibble(x = 1) %>%
  mutate(
    file_path = path(defense_figure_dir, glue("sensitivity_simulated_peak_crps_dotplot_plot"), ext = "pdf"),
...
peak_crps_dotplot_plots %>% 
  as.list() %>%
  pwalk(~ save_plot_target_asp(filename = ..1, plot = ..2, ncol = ..3, nrow = ..4, base_asp = slide_target_asp, base_height = 3))


# Bonus Figure ------------------------------------------------------------
prior_posterior_comparison_novel_variant_plot <- 
  tidy_generated_quantities_tbl %>% 
  filter(name == "prop_variant_2") %>% 
  right_join(model_table) %>% 
  group_by(data_takeover_speed) %>% 
  filter(max_t == max(max_t)) %>%
  ggplot(aes(time, value, ymin = .lower, ymax = .upper, fill = distribution, group = .width, color = distribution)) +
  facet_grid(model_description~data_takeover_speed,
             scales = "free_x",
             labeller = labeller(data_takeover_speed = ~glue("{str_to_title(.x)} Takeover Speed Data"),
                                 model_description = label_parsed)) + 
  geom_lineribbon(alpha = 0.25) +
  scale_fill_discrete("Dsitribution", labels = str_to_title) +
  scale_color_discrete("Dsitribution", labels = str_to_title) +
  scale_x_continuous("Time") +
  scale_y_continuous("Novel Variant Proportion", labels = percent) +
  theme(legend.position = "bottom") +
  ggtitle("Prior and Posterior Comparison for Novel Variant Proportion",
          subtitle = "Last Forecast Time")


save_plot_target_asp(filename = path(defense_figure_dir, "prior_posterior_comparison_novel_variant_plot", ext = "pdf"),
                     plot = prior_posterior_comparison_novel_variant_plot,
                     ncol = 3,
                     nrow = 4,
                     base_asp = slide_target_asp,
                     base_height = 1.75)
