...
@model function seirs_super_model_hifi_cdr_bin_hosp(prob, logistic_growth_time_offset, data_new_cases, data_new_deaths, data_hospitalizations, data_icu, data_new_seq, data_new_seq_variant_2, data_seq_hospitalizations,
    ...
    seq_sol_hospitalizations = seq_sol_reg_scale_array[4, 2:end]
    seq_hospitalizations_mean = seq_sol_hospitalizations

    for i in 1:l_seq_obstimes
        data_seq_hospitalizations[i] ~ NegativeBinomial2(seq_hospitalizations_mean[i], ϕ_hospitalizations)
    end

    gq = (
        seq_hospitalizations_mean=seq_hospitalizations_mean,
    )
